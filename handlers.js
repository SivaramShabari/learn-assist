...
const LaunchRequestHandler = {
    ...
    handle(handlerInput) {
        const speakOutput = 'Hi! I\'m your personal learning assistant. Ask me to teach something to get startted.';
...
const TeachIntentHandler = {
    ...
    handle(handlerInput) {
        ...
        const lesson = Alexa.getSlotValue(handlerInput.requestEnvelope, 'LESSON')
        ...
        if (subject.toLowerCase() === 'english') {
            ...
            if (lesson.toLowerCase() === 'stories') {
                console.log('english stories')
                speakOutput = propmt + ' ' + english.stories[0].content[0] + english.stories[0].content[1]
                return handlerInput.responseBuilder
                    .speak(speakOutput.replace(/ +(?= )/g, '').replace('\n', ''))
                    //.reprompt('add a reprompt if you want to keep the session open for the user to respond')
                    .getResponse();
            }
            if (lesson.toLowerCase() === 'poem') {
                console.log('english poem')
                speakOutput = propmt + ' ' + english.poems[0].content[0] + english.poems[0].content[1]
                return handlerInput.responseBuilder
                    .speak(speakOutput.replace(/ +(?= )/g, '').replace('\n', ''))
                    //.reprompt('add a reprompt if you want to keep the session open for the user to respond')
                    .getResponse();
